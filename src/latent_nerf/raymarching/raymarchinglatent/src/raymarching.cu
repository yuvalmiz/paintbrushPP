...
// image: [N, 3]
template <typename scalar_t>
__global__ void kernel_composite_rays_train_localization_forward(
    const scalar_t * __restrict__ sigmas,
    const scalar_t * __restrict__ rgbs,
    ...
    scalar_t * weights_sum,
    scalar_t * image,
    scalar_t * image_style,
    scalar_t * image_back
) {
    // parallel per ray
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= N) {
        // printf("ray number %d is bigger then N: %d\n", n,N);
        return;
    }
    ...
    // empty ray, or ray that exceed max step count.
    if (num_steps == 0 || offset + num_steps > M) {
        // printf("num_steps %d and offset+num_steps %d\n", num_steps, offset + num_steps);
        weights_sum[index] = 0;
        depth[index] = 0;
        image[index * (3+1)] = 0;
        image[index * (3+1) + 1] = 0;
        image[index * (3+1) + 2] = 0;
        image[index * (3+1) + 3] = 0;
        image_style[index * (3+1)] = 0;
        image_style[index * (3+1) + 1] = 0;
        image_style[index * (3+1) + 2] = 0;
        image_style[index * (3+1) + 3] = 0;
        image_back[index * (3+1)] = 0;
        image_back[index * (3+1) + 1] = 0;
        image_back[index * (3+1) + 2] = 0;
        image_back[index * (3+1) + 3] = 0;
        return;
    }
    ...
    loc_prob += offset;
    style_rgbs += offset * (3+1);
    beck_rgbs += offset * (3+1);

    // accumulate
    uint32_t step = 0;
    // uint32_t r_yellow = 255, g_yellow = 255, b_yellow = 0;
    scalar_t T = 1.0f;
    scalar_t r = 0, g = 0, b = 0, ws = 0, t = 0, d = 0 ;
    scalar_t  r_localization = 0, g_localization = 0, b_localization = 0, newvar_localization = 0;
    scalar_t r_stylization = 0, g_stylization = 0, b_stylization = 0 , newvar_stylization = 0;
    scalar_t r_background = 0, g_background = 0, b_background = 0, newvar_background = 0;
    scalar_t r_yellow = 0, g_yellow = 0, b_yellow = 0, newvar_yellow = 0;
    while (step < num_steps) {
        ...
        r_localization += weight * loc_prob[0] * r_yellow + weight * (1 - loc_prob[0]) * rgbs[0]; // TODO - ADD YELLOW COLOR red chanel
        g_localization += weight * loc_prob[0] * g_yellow + weight * (1 - loc_prob[0]) * rgbs[1]; // TODO - ADD YELLOW COLOR green chanel
        b_localization += weight * loc_prob[0] * b_yellow + weight * (1 - loc_prob[0]) * rgbs[2]; // TODO - ADD YELLOW COLOR blue chanel
        newvar_localization += weight * loc_prob[0] * newvar_yellow + weight * (1 - loc_prob[0]) * rgbs[3];


        r_stylization += weight * loc_prob[0] * style_rgbs[0] + weight * (1 - loc_prob[0]) * rgbs[0];
        g_stylization += weight * loc_prob[0] * style_rgbs[1] + weight * (1 - loc_prob[0]) * rgbs[1];
        b_stylization += weight * loc_prob[0] * style_rgbs[2] + weight * (1 - loc_prob[0]) * rgbs[2];
        newvar_stylization += weight * loc_prob[0] * style_rgbs[3] + weight * (1 - loc_prob[0]) * rgbs[3];


        // r_background += weight * (1-loc_prob[0]) * beck_rgbs[0] + weight * (loc_prob[0]) * rgbs[0];
        // g_background += weight * (1-loc_prob[0]) * beck_rgbs[1] + weight * (loc_prob[0]) * rgbs[1];
        // b_background += weight * (1-loc_prob[0]) * beck_rgbs[2] + weight * (loc_prob[0]) * rgbs[2];
        // newvar_background += weight * (1-loc_prob[0]) * beck_rgbs[3] + weight * (loc_prob[0]) * rgbs[3];

        r_background += weight * (1-loc_prob[0]) * beck_rgbs[0] + weight * (loc_prob[0]) * r_yellow;
        g_background += weight * (1-loc_prob[0]) * beck_rgbs[1] + weight * (loc_prob[0]) * g_yellow;
        b_background += weight * (1-loc_prob[0]) * beck_rgbs[2] + weight * (loc_prob[0]) * b_yellow;
        newvar_background += weight * (1-loc_prob[0]) * beck_rgbs[3] + weight * (loc_prob[0]) * newvar_yellow;
    ...
    weights_sum[index] = ws; // weights_sum
    depth[index] = d;
    image[index * (3+1)] = r_localization;
    image[index * (3+1) + 1] = g_localization;
    image[index * (3+1) + 2] = b_localization;
    image[index * (3+1) + 3] = newvar_localization;
    image_style[index * (3+1)] = r_stylization;
    image_style[index * (3+1) + 1] = g_stylization;
    image_style[index * (3+1) + 2] = b_stylization;
    image_style[index * (3+1) + 3] = newvar_stylization;
    image_back[index * (3+1)] = r_background;
    image_back[index * (3+1) + 1] = g_background;
    image_back[index * (3+1) + 2] = b_background;
    image_back[index * (3+1) + 3] = newvar_background;
    // printf("[n=%d] rgb=(%f, %f, %f), index: %d, image_index=(%f, %f, %f, %f)\n", n, r_localization, g_localization, b_localization, index, image[index * (3+1)], image[index * (3+1)+1], image[index * (3+1)+2], image[index * (3+1)+3]);
    
}


void composite_rays_train_localization_forward(const at::Tensor sigmas, const at::Tensor rgbs, 
                                  ...
                                  at::Tensor image_style, at::Tensor image_back) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    sigmas.scalar_type(), "composite_rays_train_localization_forward", ([&] {
        kernel_composite_rays_train_localization_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(),
                                                                                     ...
                                                                                     image_style.data_ptr<scalar_t>(),
                                                                                     image_back.data_ptr<scalar_t>());
...
template <typename scalar_t>
__global__ void kernel_composite_rays_train_localization_backward(
    const scalar_t * __restrict__ grad_weights_sum,
    const scalar_t * __restrict__ grad_image,
    ...
    const scalar_t * __restrict__ weights_sum,
    const scalar_t * __restrict__ image,
    const scalar_t * __restrict__ image_style,
    const scalar_t * __restrict__ image_back,
    const uint32_t M, const uint32_t N, const float T_thresh,
    ...
    grad_weights_sum += index;
    grad_image += index * (3+1); // TODO - check if the if train.localization is needed
    grad_image_style += index * (3+1);
    grad_image_back += index * (3+1);
    weights_sum += index;
    image += index * (3+1);
    image_style += index * (3+1);
    image_back += index * (3+1);
    sigmas += offset;
    rgbs += offset * (3+1);
    loc_prob += offset;
    style_rgbs += offset * (3+1);
    ...
    grad_rgbs_style += offset * (3+1);
    grad_rgbs_back += offset * (3+1);
    ...
    scalar_t T = 1.0f;
    const scalar_t r_final = image[0], g_final = image[1], b_final = image[2], newvar_final = image[3], ws_final = weights_sum[0];
    const scalar_t r_final_style = image_style[0], g_final_style = image_style[1], b_final_style = image_style[2], newvar_final_style = image_style[3];
    const scalar_t r_final_back = image_back[0], g_final_back = image_back[1], b_final_back = image_back[2], newvar_final_back = image_back[3];
    scalar_t r = 0, g = 0, b = 0, newvar=0, ws = 0;
    scalar_t r_yellow = 0, g_yellow = 0, b_yellow = 0, newvar_yellow = 0;
    ...
    while (step < num_steps) {
        ...
        grad_rgbs_style[0] = grad_image_style[0] * weight * loc_prob[0];
        grad_rgbs_style[1] = grad_image_style[1] * weight * loc_prob[0];
        grad_rgbs_style[2] = grad_image_style[2] * weight * loc_prob[0];
        grad_rgbs_style[3] = grad_image_style[3] * weight * loc_prob[0];

        grad_rgbs_back[0] = grad_image_back[0] * weight * ( 1- loc_prob[0]);
        grad_rgbs_back[1] = grad_image_back[1] * weight * ( 1- loc_prob[0]);
        grad_rgbs_back[2] = grad_image_back[2] * weight * ( 1- loc_prob[0]);
        grad_rgbs_back[3] = grad_image_back[3] * weight * ( 1- loc_prob[0]);

        // write grad_sigmas

        grad_loc_prob[0] = grad_image[0] * weight * (r_yellow - rgbs[0]) +
                           grad_image[1] * weight * (g_yellow - rgbs[1]) +
                           grad_image[2] * weight * (b_yellow - rgbs[2]) +
                           grad_image[3] * weight * (newvar_yellow - rgbs[3]) +
                        //    grad_image[3] * (T * rgbs[3] - (newvar_final - newvar)) TODO - add newvar gradient
                           grad_image_style[0] * weight * (style_rgbs[0] - rgbs[0]) +
                           grad_image_style[1] * weight * (style_rgbs[1] - rgbs[1]) +
                           grad_image_style[2] * weight * (style_rgbs[2] - rgbs[2]) +
                           grad_image_style[3] * weight * (style_rgbs[3] - rgbs[3]) +
                        ...
                        //    grad_image_back[3] * weight * (rgbs[3] - beck_rgbs[3]);
                            grad_image_back[0] * weight * (r_yellow - beck_rgbs[0]) +
                            grad_image_back[1] * weight * (g_yellow - beck_rgbs[1]) +
                            grad_image_back[2] * weight * (b_yellow - beck_rgbs[2])+
                            grad_image_back[3] * weight * (newvar_yellow - beck_rgbs[3]);
...
void composite_rays_train_localization_backward(const at::Tensor grad_weights_sum, const at::Tensor grad_image,
                                ...
                                const at::Tensor image, const at::Tensor image_style, const at::Tensor image_back,
                                const uint32_t M, const uint32_t N, const float T_thresh,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_image.scalar_type(), "composite_rays_train_localization_backward", ([&] {
        kernel_composite_rays_train_localization_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(grad_weights_sum.data_ptr<scalar_t>(),
...
template <typename scalar_t>
__global__ void kernel_composite_rays_localization(
    const uint32_t n_alive,
    const uint32_t n_step,
    const float T_thresh,
    int* rays_alive,
    ...
    scalar_t* image, scalar_t* image_style, scalar_t* image_back
) {
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= n_alive) return;
    ...
    loc_prob += n * n_step;
    rgbs += n * n_step * (3+1);
    style_rgbs += n * n_step * (3+1);
    beck_rgbs += n * n_step * (3+1);
    deltas += n * n_step * 2;
    ...
    scalar_t weight_sum = weights_sum[0];
    scalar_t d = depth[0];
    scalar_t r_localization = image[0];
    scalar_t g_localization = image[1];
    scalar_t b_localization = image[2];
    scalar_t newvar_localization = image[3];
    
    scalar_t r_stylization = image_style[0];
    scalar_t g_stylization = image_style[1];
    scalar_t b_stylization = image_style[2];
    scalar_t newvar_stylization = image_style[3];

    scalar_t r_background = image_back[0];
    scalar_t g_background = image_back[1];
    scalar_t b_background = image_back[2];
    scalar_t newvar_background = image_back[3];
    ...
    image[0] = r_localization;
    image[1] = g_localization;
    image[2] = b_localization;
    image[3] = newvar_localization;

    image_style[0] = r_stylization;
    image_style[1] = g_stylization;
    image_style[2] = b_stylization;
    image_style[3] = newvar_stylization;

    image_back[0] = r_background;
    image_back[1] = g_background;
    image_back[2] = b_background;
    image_back[3] = newvar_background;
}


void composite_rays_localization(const uint32_t n_alive, const uint32_t n_step, const float T_thresh,
                                 ...
                                 at::Tensor image, at::Tensor image_style, at::Tensor image_back) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "composite_rays_localization", ([&] {
        kernel_composite_rays_localization<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh,
...
__global__ void kernel_composite_rays_train_localization_int_forward(
    ...
    if (num_steps == 0 || offset + num_steps > M) {
        ...
        image_origin[index * (3+1) + 1] = 0;
        image_origin[index * (3+1) + 2] = 0;
        image_origin[index * (3+1) + 3] = 0;
        
        image_prob[index] = 0;

        image_style_origin[index * (3+1)] = 0;
        image_style_origin[index * (3+1) + 1] = 0;
        image_style_origin[index * (3+1) + 2] = 0;
        image_style_origin[index * (3+1) + 3] = 0;

        image_back_origin[index * (3+1)] = 0;
        image_back_origin[index * (3+1) + 1] = 0;
        image_back_origin[index * (3+1) + 2] = 0;
        image_back_origin[index * (3+1) + 3] = 0;

        image_localization_mix[index * (3+1)] = 0;
        image_localization_mix[index * (3+1) + 1] = 0;
        image_localization_mix[index * (3+1) + 2] = 0;
        image_localization_mix[index * (3+1) + 3] = 0;

        image_style_mix[index * (3+1)] = 0;
        image_style_mix[index * (3+1) + 1] = 0;
        image_style_mix[index * (3+1) + 2] = 0;
        image_style_mix[index * (3+1) + 3] = 0;

        image_back_mix[index * (3+1)] = 0;
        image_back_mix[index * (3+1) + 1] = 0;
        image_back_mix[index * (3+1) + 2] = 0;
        image_back_mix[index * (3+1) + 3] = 0;
        return;
    }
    ...
    // uint32_t r_yellow = 255, g_yellow = 255, b_yellow = 0;
    scalar_t T = 1.0f;
    scalar_t r = 0, g = 0, b = 0, ws = 0, t = 0, d = 0, newvar = 0 ;
    ...
    while (step < num_steps) {
        ...
        newvar_stylization += weight * style_rgbs[3];
        ...
        newvar_background += weight * beck_rgbs[3];
    ...
    image_origin[index * (3+1)] = r;
    image_origin[index * (3+1) + 1] = g;
    image_origin[index * (3+1) + 2] = b;
    image_origin[index * (3+1) + 3] = newvar;
    if (localization_prob > 1){
        printf("%f",localization_prob);
    }
    image_prob[index] = localization_prob;

    image_style_origin[index * (3+1)] = r_stylization;
    image_style_origin[index * (3+1) + 1] = g_stylization;
    image_style_origin[index * (3+1) + 2] = b_stylization;
    image_style_origin[index * (3+1) + 3] = newvar_stylization;

    image_back_origin[index * (3+1)] = r_background;
    image_back_origin[index * (3+1) + 1] = g_background;
    image_back_origin[index * (3+1) + 2] = b_background;
    image_back_origin[index * (3+1) + 3] = newvar_background;

    image_localization_mix[index * (3+1)] = localization_prob * r_yellow + (1 - localization_prob) * r;
    image_localization_mix[index * (3+1) + 1] = localization_prob * g_yellow + (1 - localization_prob) * g;
    image_localization_mix[index * (3+1) + 2] = localization_prob * b_yellow + (1 - localization_prob) * b;
    image_localization_mix[index * (3+1) + 3] = localization_prob * newvar_yellow + (1 - localization_prob) *  newvar;
    
    image_style_mix[index * (3+1)] = localization_prob * r_stylization + (1 - localization_prob) * r;
    image_style_mix[index * (3+1) + 1] = localization_prob * g_stylization + (1 - localization_prob) * g;
    image_style_mix[index * (3+1) + 2] = localization_prob * b_stylization + (1 - localization_prob) * b;
    image_style_mix[index * (3+1) + 3] = localization_prob * newvar_stylization + (1 - localization_prob) * newvar;

    image_back_mix[index * (3+1)] = localization_prob * r_yellow + (1 - localization_prob) * r_background;
    image_back_mix[index * (3+1) + 1] = localization_prob * g_yellow + (1 - localization_prob) * g_background;
    image_back_mix[index * (3+1) + 2] = localization_prob * b_yellow + (1 - localization_prob) * b_background;
    image_back_mix[index * (3+1) + 3] =  localization_prob * newvar_yellow + (1 - localization_prob) * newvar_background;
    // printf("[n=%d] rgb=(%f, %f, %f), index: %d, image_index=(%f, %f, %f, %f)\n", n, r_localization, g_localization, b_localization, index, image[index * (3+1)], image[index * (3+1)+1], image[index * (3+1)+2], image[index * (3+1)+3]);
}
...
__global__ void kernel_composite_rays_train_localization_int_backward(
    ...
    image_origin += index * (3+1);

    image_prob += index;

    image_style_origin += index * (3+1);

    image_back_origin += index * (3+1);

    image_localization_mix += index * (3+1);

    image_style_mix += index * (3+1);

    image_back_mix += index * (3+1);
    ...
    while (step < num_steps) {
        ...
        grad_rgbs_style[3] = grad_image_style[3] * weight * image_prob[0];
        ...
        grad_rgbs_back[3] = grad_image_back[3] * weight * ( 1- image_prob[0]);

        // write grad_sigmas

        grad_loc_prob[0] = grad_image[0] * weight * (r_yellow - image_origin[0]) +
                           grad_image[1] * weight * (g_yellow - image_origin[1]) +
                           grad_image[2] * weight * (b_yellow - image_origin[2]) +
                           grad_image[3] * weight * (newvar_yellow - image_origin[3]) +
                        //    grad_image[3] * (T * rgbs[3] - (newvar_final - newvar)) TODO - add newvar gradient
                           grad_image_style[0] * weight * (image_style_origin[0] - image_origin[0]) +
                           grad_image_style[1] * weight * (image_style_origin[1] - image_origin[1]) +
                           grad_image_style[2] * weight * (image_style_origin[2] - image_origin[2]) +
                           grad_image_style[3] * weight * (image_style_origin[3] - image_origin[3]) +
                           // grad_image_style[3] * weight * (style_rgbs[3] - rgbs[3]) + TODO - add newvar gradient
                        //    grad_image_back[0] * weight * (rgbs[0] - beck_rgbs[0]) +
                        //    grad_image_back[1] * weight * (rgbs[1] - beck_rgbs[1]) +
                        //    grad_image_back[2] * weight * (rgbs[2] - beck_rgbs[2])+
                        //    grad_image_back[3] * weight * (rgbs[3] - beck_rgbs[3]);
                            ...
                            grad_image_back[3] * weight * (newvar_yellow - image_back_origin[3]);
                           // grad_image_back[3] * weight * (rgbs[3] - beck_rgbs[3]); TODO - add newvar gradient
...
__global__ void kernel_composite_rays_localization_int(
    ...
    scalar_t newvar = image_origin[3];
    ...
    scalar_t newvar_stylization = image_style_origin[3];
    ...
    scalar_t newvar_background = image_back_origin[3];
    ...
    image_style_origin[3] = newvar_stylization;

    image_back_origin[0] = r_background;
    image_back_origin[1] = g_background;
    image_back_origin[2] = b_background;
    image_back_origin[3] = newvar_background;

    image_localization_mix[0] = localization_prob * r_yellow + (1 - localization_prob) * r;
    image_localization_mix[1] = localization_prob * g_yellow + (1 - localization_prob) * g;
    image_localization_mix[2] = localization_prob * b_yellow + (1 - localization_prob) * b;
    image_localization_mix[3] = localization_prob * newvar_yellow + (1 - localization_prob) *  newvar;
    
    image_style_mix[0] = localization_prob * r_stylization + (1 - localization_prob) * r;
    image_style_mix[1] = localization_prob * g_stylization + (1 - localization_prob) * g;
    image_style_mix[2] = localization_prob * b_stylization + (1 - localization_prob) * b;
    image_style_mix[3] = localization_prob * newvar_stylization + (1 - localization_prob) * newvar;
    ...
    image_back_mix[3] =  localization_prob * newvar_yellow + (1 - localization_prob) * newvar_background;
}
